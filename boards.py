...
class Boards(object):
    ...
    def delete(self, board_id):
        resp = requests.delete("https://api.trello.com/1/boards/%s" % (board_id), params=dict(key=self._apikey, token=self._token), data=None)
    ...
    def new(self, name, desc=None, idOrganization=None, idBoardSource=None):
        resp = requests.post("https://trello.com/1/boards" % (), params=dict(key=self._apikey, token=self._token), data=dict(name=name, desc=desc, idOrganization=idOrganization, idBoardSource=idBoardSource))
        resp.raise_for_status()
        return json.loads(resp.content)
    ...
    def new_member(self, board_id, email):
        resp = requests.post("https://trello.com/1/boards/%s/members" % (board_id), params=dict(key=self._apikey, token=self._token), data=dict(board_id=board_id, email=email))
        resp.raise_for_status()
        return json.loads(resp.content)
