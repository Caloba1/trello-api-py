...
class Cards(object):
    ...
    def get_custom_field(self, card_id):
        resp = requests.get("https://trello.com/1/cards/%s/customFieldItems/%s" % (card_id), params=dict(key=self._apikey, token=self._token), data=None)
        resp.raise_for_status()
        return json.loads(resp.content)
    ...
    def update(self, card_id, name=None, desc=None, closed=None, idList=None, due=None, idBoard=None, pos=None):
        resp = requests.put("https://trello.com/1/cards/%s" % (card_id), params=dict(key=self._apikey, token=self._token), data=dict(name=name, desc=desc, closed=closed, idList=idList, due=due, idBoard=idBoard, pos=pos))
        resp.raise_for_status()
        return json.loads(resp.content)

    def update_checkItem(self, card_id, item_id, name=None, state=None, pos=None):
        resp = requests.put("https://trello.com/1/cards/%s/checkItem/%s" % (card_id, item_id), params=dict(key=self._apikey, token=self._token), data=dict(name=name, state=state, pos=pos))
        resp.raise_for_status()
        return json.loads(resp.content)
    ...
    def new_checklist(self, card_id, name, idChecklistSource=None, pos=None):
        resp = requests.post("https://trello.com/1/cards/%s/checklists" % (card_id), params=dict(key=self._apikey, token=self._token), data=dict(name=name, idChecklistSource=idChecklistSource, pos=pos))
        resp.raise_for_status()
        return json.loads(resp.content)
    ...
    def new_membersVoted(self, card_id, value):
        ...
        return json.loads(resp.content)
    #--------------------------------------------------
    def new_sticker(self, card_id, image, top, left, z_index, rotate=None):
        resp = requests.post("https://trello.com/1/cards/%s/stickers" % (card_id), params=dict(key=self._apikey, token=self._token), data=dict(image=image, top=top, left=left,z_index=z_index, rotate=rotate))
        resp.raise_for_status()
        return json.loads(resp.content)
