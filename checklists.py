...
class Checklists(object):
    ...
    def new_checkItem(self, checklist_id, name, pos=None, checked=None):
        resp = requests.post("https://trello.com/1/checklists/%s/checkItems" % (checklist_id), params=dict(key=self._apikey, token=self._token), data=dict(name=name, pos=pos, checked=checked))
        resp.raise_for_status()
        return json.loads(resp.content)
    
    def delete_checkList(self, checklist_id):
        resp = requests.delete("https://trello.com/1/checklists/%s/" % (checklist_id), params=dict(key=self._apikey, token=self._token), data=None)
        resp.raise_for_status()
        return json.loads(resp.content)
